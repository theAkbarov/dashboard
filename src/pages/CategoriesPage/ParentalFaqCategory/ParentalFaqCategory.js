...
const ParentalFaqCategory = () => {
  ...
  const [deleteInfographics, deleteInfographicsInfo] = useMutation(
    requests.category.parental.faq.delete,
    {
      onSuccess: () => {
        ...
        modal.close();
        cache.invalidateQueries("parentalFaqCategoryList");
      },
      onError: () => {
        notification.error({
  ...
  return (
    <Spin
      spinning={parentalFaqCatList.isLoading || parentalFaqCatList.isFetching}
    >
      <CustomModal
      ...
      <Card>
        <Table
          ...
          columns={[
            {
              ...
              render: (text, record, index) =>
                getRowNum(
                  index,
                  1,
                  parentalFaqCatList?.data?.data?.results?.length,
                  1000
                ),
            },
            {
              ...
              render: (_, record) => (
                <Space>
                  <Button
                    ...
                    icon={<EditOutlined />}
                    onClick={() => {
                      history.push(`categories/parental-faq/${record.guid}`);
                    }}
                  />
                  <Button
                    type="link"
          ...
          dataSource={_.get(parentalFaqCatList, ["data", "data", "results"])}
        />
      </Card>
    </Spin>
  );
