...
const ParentalFaqCategoryDetails = () => {
  ...
  const [addCategory, addCategoryInfo] = useMutation(
    params.id && params.id !== "create"
      ? requests.category.parental.faq.update
      : requests.category.parental.faq.create,
    {
      onSuccess: (mes) => {
        ...
        cashe.invalidateQueries(["parentalFaqCategory", params.id]);
        history.push("/categories?activeTab=parental-faq");
      },
      onError: () => {
        notification.error({
  ...
  const categoryItem = useQuery(
    ["parentalFaqCategory", params.id],
    () => requests.category.parental.faq.getSingle(params.id),
...
export default ParentalFaqCategoryDetails;
